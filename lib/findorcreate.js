...
function sanitizeMongoKeys(query) {
  if (typeof query !== "object" || query instanceof Date || query === null)
    return query;
  ...
  schema.statics.findOrCreate = function findOrCreate(
    ...
    this.findOne(query).exec((err, result) => {
      ...
      if (result && !additionalFields)
        return callback(err, result, false, false);

      const creating = result ? false : true;
      const doc = creating ? new this(sanitizeMongoKeys(query)) : result;

      if (!creating && !options.saveIfFound)
        return callback(err, doc, false, creating);
      ...
      if (!doc.isModified() && !creating)
        return callback(err, doc, false, creating);

      doc.save(options.saveOptions, (err) =>
        callback(err, doc, true, creating)
      );
    });
